...
**待人工操作**
- [x] Zeabur 部署 `/backend`（Docker / FastAPI / uvicorn）— `https://flight-search-web.zeabur.app`
- [x] Cloudflare Pages 部署 `/frontend` — `https://flight-search-web-29x.pages.dev`
...
| 🟡 中 | 隔日檢查排程是否寫入三條航線歷史 | 排程驗收 |
| 🟡 中 | 線上熔斷演練（暫壞 fast-flights → Kiwi） | 最終驗收 |

已完成（2026-07-07）：Supabase 專案 `flight-search` 建立＋schema v1–v3 套用＋RLS 啟用；`airports.csv` 下載＋`airports.json` 產出（275.7 KB）；API Token 產生；備援 provider Amadeus → Kiwi.com 轉向；Zeabur 後端與 Cloudflare Pages 前端已部署並通過 smoke verification。
