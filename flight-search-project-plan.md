...
| 項目 | 內容 |
|---|---|
| 版本 | v2.2（proxy 降為純選配，新增 $0 零 Proxy 生存策略） |
| 日期 | 2026-07-06 |
...
4️⃣ SerpAPI（選配，免費 100 次/月，僅緊急兜底）
...
- providers/fast_flights_provider.py：fast-flights 套件，
  asyncio.Semaphore(1) 單併發＋每次請求前 sleep 3-6 秒隨機 jitter
  （偽裝人類節奏，這是零 proxy 策略核心）；HTTPS_PROXY 環境變數
  留鉤子但預設不用
...
- 中介層驗 X-API-Token（環境變數 API_TOKEN），不符回 403；
  /api/health 免 token（給 UptimeRobot）
...
| 9 | API 防護 | Token + IP rate limit，防白嫖拖垮配額 |
...
| 排程沒跑 | 容器重啟時點 | 查 Zeabur log；APScheduler misfire_grace_time 設 3600 |
...
1. **回滾**：`git reset --hard phase-N-done`（查核點＝存檔點），評估是否重來比硬修快
...
*v2.0 文件結束。各 Phase 查核點通過後於本文件打勾並 git tag，作為專案完成紀錄。*
...
**E7. provider_status schema**（Phase 3 的 db/schema_v2.sql 即此表，與附錄 A 一致）：`provider (pk), state, failure_count, opened_at, monthly_calls, month_key, last_success_at`
...
*v2.1 修訂完畢：原兩個人工把關點已分別由附錄 E（預審定規格）與附錄 F（機械化閘門）取代。*
